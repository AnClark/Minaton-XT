 ...
 *
 *
 *  This is a modified version of Dear ImGui binary_to_compressed_c.cpp for Minaton-XT.
 *  It process a directory instead of single file. Useful for batch converting resources
 *  into C code, with compression and/or Base85 encode.
 */
...
// Usage:
//   binary_to_compressed_c.exe [-base85] [-nocompress] <inputdir> <symbolname>
// Usage example:
...
static void scan_files(const std::string dir_path, std::vector<std::string>& filenames);

static bool binary_to_compressed_c(const char* filename, const char* symbol, bool use_base85_encoding, bool use_compression);
static bool batch_convert(const char* input_dir, const char* symbol, bool use_base85_encoding, bool use_compression);

int main(int argc, char** argv)
{
    if (argc < 3) {
        printf("Syntax: %s [-base85] [-nocompress] <inputdir> <symbolname>\n", argv[0]);
        return 0;
    }
    ...
    bool ret = batch_convert(argv[argn], argv[argn + 1], use_base85_encoding, use_compression);
    if (!ret)
...
void scan_files(const std::string dir_path, std::vector<std::string>& filenames)
{
#if _WIN32
...
bool batch_convert(const char* input_dir, const char* symbol, bool use_base85_encoding, bool use_compression)
{
    std::vector<std::string> input_filenames;
    scan_files(std::string(input_dir), input_filenames);
    ...
    for (auto input_filename = input_filenames.begin(); input_filename != input_filenames.end(); input_filename++) {
        fprintf(stderr, "* Converting: %s\n", input_filename->c_str());

        file_path = std::string(input_dir) + "/" + *input_filename;

        std::string file_name_remove_dot(*input_filename);
        std::replace(file_name_remove_dot.begin(), file_name_remove_dot.end(), '.', '_');
        std::replace(file_name_remove_dot.begin(), file_name_remove_dot.end(), '-', '_');
        symbol_concat = std::string(symbol) + std::string("_") + file_name_remove_dot;

        ret = binary_to_compressed_c(file_path.c_str(), symbol_concat.c_str(), use_base85_encoding, use_compression);
        if (!ret) {
            fprintf(stderr, "Error opening or reading file: '%s'\n", input_filename->c_str());
            return false;
        }
