...
# NOTICE:
# This version of res2c.py added paramter "prefix", which adds prefix to symbols.
# Useful if filenames includes leading characters not matching [A-Za-z_].

import os, sys

# -----------------------------------------------------

def res2c(namespace, prefix, filenames):
    ...
    for filename in filenames:
        ...
        fdH.write("    extern const unsigned char* %s%sData;\n" % (prefix, shortFilename))
        fdH.write("    const unsigned int %s%sDataSize = %i;\n" % (prefix, shortFilename, len(resData)))
        ...
        fdC.write("};\n")
        fdC.write("const unsigned char* %s::%s%sData = (const unsigned char*)temp_%s_%i;\n" % (namespace, prefix, shortFilename, shortFilename, tempIndex))
...
if __name__ == '__main__':
    if len(sys.argv) not in (4, 5):
        print("Usage: %s <namespace> <prefix> <resource-folder> [output-folder=$CWD]" % sys.argv[0])
        quit()

    namespace = sys.argv[1].replace("-","_")
    prefix = sys.argv[2]
    resFolder = sys.argv[3]
    outFolder = sys.argv[4] if len(sys.argv) == 5 else None
    ...
    # create code now
    res2c(namespace, prefix, resFiles)
