...
void MinatonPlugin::run(const float** inputs, float** outputs, uint32_t frames, const MidiEvent* midiEvents, uint32_t midiEventCount)
{
    ...
    if (fSampleRate == 44100.0f) {
        ...
        uint64_t targetSampleCount = Resample_f32(buffer_before_resample_l, NULL, 44100, int(fSampleRate), expect_input_size, channels);
        d_stderr("Required output size: %d, expected input size: %f", targetSampleCount, expect_input_size);
#endif
